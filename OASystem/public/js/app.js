...
app.controller("infoController", function($scope, $sce, $http){
    ...
    $http.get('/info', {responseType: 'arraybuffer'})//QUESTION: what does this return? Blob or generalInfo object?
...
app.controller("questionController", function($scope, $http){
    ...
    $http.get(url).success(function (data){
        ...
        for(var i=0; i<data.length; i++){
        	questions.push(data[i].question.qContent);
        }
    ...
    $scope.searchQuestion = function(){
        ...
        if($scope.searchButtonName == "Search"){
            ...
            var url = "/questions/search?name="+ searchQ;
            $http.post(url).success(function (data){
            	console.log("answer is:  "+data);
            	$("#searchResult").val() = "Answer goes here";
            }).error(function(data){
                console.log("Unable to search question");
            });
            //display result here
